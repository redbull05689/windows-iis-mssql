...
function Check-Chocolatey {
    ...
    if(-not $env:ChocolateyInstall -or -not (Test-Path "$env:ChocolateyInstall")){
        ...
        if($Force -OR (Confirm-Install)){
            ...
            if($exitCode -ne 0) {
                #causing an error on vagrant provisions
                #Write-Warning ".net install returned $exitCode. You likely need to reboot your computer before proceeding with the install."
                Write-Host ".net install returned $exitCode. You likely need to reboot your computer before proceeding with the install."
                return $false
            }
