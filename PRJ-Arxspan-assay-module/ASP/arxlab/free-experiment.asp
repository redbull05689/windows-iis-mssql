...
%>
<!-- #include file="_inclds/experiments/free/asp/freeGetExperimentVars.asp"-->
...
<%If (canViewExperiment("3",experimentId,session("userId")) Or canRead Or hasInviteRead) And notebookVisible And experimentVisible then%>
<!-- #include file="experiments/fileUploadForm.asp"-->
<!-- #include file="_inclds/common/html/infoBox.asp"-->

<!-- #include file="_inclds/experiments/free/js/freeJSIncludes.asp"-->
<!-- #include file="_inclds/experiments/common/asp/unsavedChanges.asp"-->
<!-- #include file="_inclds/experiments/free/js/freeJS.asp"-->

<!-- #include file="_inclds/experiments/free/html/freeForm.asp"-->
...
%>
<%
if numAttachments > 0 and trim(removeTags(description)) = "" then
%>
	try{addEvent(window,'load',function(){showMainDiv('attachmentTable')})}catch(err){}
<%
end if
%>
<%
