...
<script type="text/javascript">
        ...
        for (i=0;i<mainTabs.length ;i++ )
        {
            ...
            else
            {
                try
                {
                    ...
                    if (currentMainTab == "experimentDiv") {
                        var iframe = $("#tocIframe")[0]
                        iframe.contentWindow.utilities().resizeCustExpIframe();
                    }
                    try
                    {
    ...
    var experimentTypeName = "<%=experimentTypeName%>"

	// include the file that contains the server save function
	<!-- #include file="../../common/js/serverSaveExperimentJS.asp"-->

    function experimentSubmit(approve,sign,autoSave,refreshPageAfterSave, requestId, workflowRevisionId, afterSaveCallbackFn = function(){})
    {
        showPopup('savingDiv')
        killIntervals();
        if(sign){
		...
		experimentJSON["requestTypeId"] = $("#requestTypeId").val();
        experimentJSON["section_Request_sortOrder"] = -5;
		
		console.log("page requestId: ", requestId);
		console.log("experimentJSON['requestId']: ", experimentJSON["requestId"]);
		
		console.log("page requestTypeId: ", $("#requestTypeId").val());
		console.log("experimentJSON['requestTypeId']: ", experimentJSON["requestTypeId"]);
		...
		doServerSaveExperiment("<%=mainAppPath%>/experiments/cust-saveExperiment.asp",{hiddenExperimentJSON: JSON.stringify(hiddenExperimentJSON), canWrite: canWrite },sign, null, null, afterSaveCallbackFn);		
    }
...
<%If request.querystring("revisionId") = "" and (ownsExp or canWrite) Then%>
