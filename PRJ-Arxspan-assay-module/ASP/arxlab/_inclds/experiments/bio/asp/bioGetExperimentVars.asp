...
If request.querystring("id") <> "" then
	...
	call getconnected
	If revisionId = "" then
		strQuery22 = "SELECT * FROM bioExperimentsView WHERE id=" & SQLClean(request.querystring("id"),"N","S")
		currentRevisionNumber = getExperimentRevisionNumber(experimentType,experimentId)
	Else
		strQuery22 = "SELECT * FROM bioExperimentHistoryView WHERE experimentId=" & SQLClean(request.querystring("id"),"N","S") & " AND revisionNumber="&SQLClean(revisionId,"N","S")
	End If
	...
	If Not expRec.eof Then
		...
		If IsNull(experimentDetails) Then
			experimentDetails = ""
		End if
		summary = expRec("summary")
		protocol = expRec("protocol")
		...
		If statusId = 5 Or statusId = 6 or statusId = 10 Or statusId = 11 Then
			If session("justSaved")=True Then
				...
				response.redirect("dashboard.asp?id="&experimentId&"&experimentType=2&revisionNumber="&currentRevisionNumber)
			Else
				If session("redirectToSignedPDF") Then
					If request.querystring("expView") = "" then
						'If currentRevisionNumber <> 1 then
							response.redirect("signed.asp?id="&experimentId&"&experimentType=2&revisionNumber="&currentRevisionNumber)
						'End If
					End if
				End if
				' if its singed/witnessed and they try to edit the page by entering no revisionId, then 302 them to the read only version
				If request.querystring("revisionId") = "" then
					response.redirect("bio-experiment.asp?id=" & experimentId & "&revisionId=" & currentRevisionNumber & "&expView=" & request.querystring("expView"))
				End if
