...
<form action="<%=mainAppPath%>/experiments/bio-saveExperiment.asp" method="post" id="experimentForm" onsubmit="return false;">
<input type="hidden" name="hungSaveSerial" value="<%=hungSaveSerial%>">
  <table cellpadding="0" cellspacing="0" style="width:100%;">
	...
	<tr>
		<td align="left" style="background-color:transparent;padding-bottom:none!important;" valign="top" colspan="2">
			<div class="elnObjectContainer elnDashObj">
				<div class="tabs elnHead"><h2><%If whichClient = "CRISPR" Then%>Purpose<%else%><%=experimentDescriptionLabel%><%end if%><span class="requiredExperimentFieldNotice" data-fieldname="e_details">*</span></h2></div>
				<table cellpadding="0" cellspacing="0" style="width:100%;">
					<tr>
						<%If revisionId = "" and canWrite And ownsExp then%>
	...
	<tr <%If experimentId = "" then%>style="display:none;"<%End if%> id="sectionRow">
		<td>
			...
			<li><a href="javascript:void(0);" onClick="showMainDiv('attachmentTable');return false;" id="attachmentTable_tab" style="<%if Not hasAttachments then%>display:none;<%End if%>"><%=attachmentsTableLabel%></a></li>				<li><a href="javascript:void(0);" onClick="showMainDiv('noteTable');return false;" id="noteTable_tab" style="<%If Not hasNotes then%>display:none;<%End if%>"><%=notesTableLabel%></a></li>
			<%If revisionId = "" And canWrite And ownsExp then%>
  ...
  <table cellpadding="0" cellspacing="0" style="table-layout:fixed;width:100%;<%If experimentId= "" then%>display:none;<%End if%>">
	<tr>
		<td align="left" style="padding-bottom:0!important;" valign="top" colspan="2">
			<div class="elnObjectContainer elnDashObj">
				<div class="tabs elnHead"><h2><%=protocolLabel%></h2></div>
				<table cellpadding="0" cellspacing="0" style="width:100%;">
					<tr>
						<td class="caseInnerTitle expBoxText" valign="top">
							<%If revisionId = "" and canWrite And ownsExp then%>
								...
								<script type="text/javascript">
									...
									CKEDITOR.instances['e_protocol'].on('resize',function(){positionButtons();updateCkEditorSize('bioform_e_protocol_height','e_protocol');})
									...
									if(userOptions['bioform_e_protocol_height']){
										CKEDITOR.instances['e_protocol'].config.height = userOptions['bioform_e_protocol_height'];
									}
									$(function(){
	...
	<tr>
		<td align="left" style="padding-bottom:0!important;" valign="top" colspan="2">
			<div class="elnObjectContainer elnDashObj">
				<div class="tabs elnHead"><h2><%If whichClient = "CRISPR" Then%>Results<%else%><%=summaryLabel%><%end if%></h2></div>
				<table cellpadding="0" cellspacing="0" style="width:100%;">
					<tr>
						<td class="caseInnerTitle expBoxText" valign="top">
							<%If revisionId = "" and canWrite And ownsExp then%>
								...
								<script type="text/javascript">
									...
									CKEDITOR.instances['e_summary'].on('resize',function(){positionButtons();updateCkEditorSize('bioform_e_summary_height','e_summary');})
									...
									if(userOptions['bioform_e_summary_height']){
										CKEDITOR.instances['e_summary'].config.height = userOptions['bioform_e_summary_height'];
									}
									$(function(){
										//Force reload the data after the page is loaded for a weird bug in firefox
			...
			<div class="elnObjectContainer elnDashObj">
				<div class="invButtons">
					<%If revisionId = "" And ownsExp then%>
						<a href="javascript:void(0)" onClick="showPopup('inventoryLinkSelectorDiv');" id="inventoryLinkLink" title="New Inventory Link"><img border="0" src="images/Add.gif" class="png" style="position:absolute;right:64px;"></a>
						<%If session("hasBarcodeChooser") then%>
							<a href="javascript:void(0)" onClick="showMultiAddInventoryPopup();return false;" id="inventoryLinkLink" title="Inventory Barcode Chooser"><img border="0" src="images/barcode.gif" class="png" height="16" width="20" style="margin-top:6px;position:absolute;right:25px;"></a>
						<%End if%>
					<%End if%>
				</div>
	...
	<%If session("regRoleNumber") <> "None" then%>
	<tr>
		<td align="left" style="padding-bottom:0!important;" valign="top" colspan="2">
			<div class="elnObjectContainer elnDashObj">
			<div class="invButtons">
