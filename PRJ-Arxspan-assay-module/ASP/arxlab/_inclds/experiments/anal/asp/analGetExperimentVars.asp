...
If request.querystring("id") <> "" then
	...
	call getconnected
	If revisionId = "" then
		strQuery22 = "SELECT * FROM analExperimentsView WHERE id=" & SQLClean(request.querystring("id"),"N","S")
		currentRevisionNumber = getExperimentRevisionNumber(experimentType,experimentId)
	Else
		strQuery22 = "SELECT * FROM analExperimentHistoryView WHERE experimentId=" & SQLClean(request.querystring("id"),"N","S") & " AND revisionNumber="&SQLClean(revisionId,"N","S")
	End If
	...
	If Not expRec.eof Then
		...
		If statusId = 5 Or statusId = 6 or statusId = 10 Or statusId = 11 Then
			If session("justSaved")=True Then
				...
				response.redirect("dashboard.asp?id="&experimentId&"&experimentType=4&revisionNumber="&currentRevisionNumber)
			Else
				If session("redirectToSignedPDF") Then
					If request.querystring("expView") = "" then
						'If currentRevisionNumber <> 1 then
							response.redirect("signed.asp?id="&experimentId&"&experimentType=4&revisionNumber="&currentRevisionNumber)
						'End If
					End if
				End if
				If request.querystring("revisionId") = "" then
					response.redirect("anal-experiment.asp?id="&experimentId&"&revisionId="&currentRevisionNumber & "&expView=" & request.querystring("expView"))
				End if
			End If
		End If
		session("justSaved") = False
