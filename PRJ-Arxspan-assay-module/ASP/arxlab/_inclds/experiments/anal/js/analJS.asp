...
<script type="text/javascript">
    ...
    function experimentSubmit(approve,sign,autoSave,refreshPageAfterSave)
    {
		...
		*/
		...
		doServerSaveExperiment("<%=mainAppPath%>/experiments/anal-saveExperiment.asp",{hiddenExperimentJSON: JSON.stringify(hiddenExperimentJSON) },sign);		
    }
    ...
    function updateElementalMachinesTable(){
         $.get("<%=mainAppPath%>/experiments/ajax/load/getElementalMachinesTable.asp?experimentId="+experimentId+"&experimentType="+experimentType+"&time="+$.now(), function(data) {
            //console.log(data);
            $("#elementalMachinesTable").html(data);
         });
    }
