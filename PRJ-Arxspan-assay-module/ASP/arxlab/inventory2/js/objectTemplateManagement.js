...
function updateAllThingsConditional(){
	...
	$('#objectFieldsTable .baseRow').each(function(){
		...
		var fieldType = $('[baserowid="'+baseRowId+'"] input[type="hidden"][jsonfield="formType"]').val();
		
		//alert(fieldName + ' conditional: ' + fieldIsConditional + ' type: ' + fieldType );
	...
	//alert('allConditionalFieldsByName
	...
	$('#objectFieldsTable .baseRow').each(function(){
		...
		//alert(conditionalFieldsDropdown_modified_HTML);
...
function addOptionToField(baseRowId, optionValue="", optionId="", conditionalFieldsDropdownValues="", conditionalFieldsDropdownOptionValues=""){
	...
	//var name = $('.baseRow[baserowid="'+baseRowId+'"] input[type="text"][jsonfield="fieldName"]').val();
	var newOptionRowHTML = '<tr><td><div class="optionValueContainer"><input type="text" id="optVal_'+baseRowId+optinx+'" class="optionValueInput" placeholder="Option Value" value="'+optionValue+'" optionIndex="'+optinx+'"></div></td><td><div class="optionIdContainer"><input type="text" id="optInp_'+baseRowId+optinx+'" class="optionIdInput" value="'+optionValue+'" DISABLED></div></td><td><div class="conditionalFieldsDropdownContainer"><select class="conditionalFieldsDropdown" multiple="" setvalueafterpageloads="'+conditionalFieldsDropdownValues+'"></select></div></td><td><div class="conditionalFieldsDropdownContainer"><select class="conditionalFieldsOptionsDropdown" multiple="" setvalueafterpageloads="'+conditionalFieldsDropdownOptionValues+'"></select></div></td><td class="removeOptionButton"></td></tr>'

//	$('body').on('change','#optInp_'+baseRowId+''+optinx,function(event){
//		alert('foo');
//	});
	

	$('body').on('change','#optVal_'+baseRowId+''+optinx,function(event){
		var inx = $(this).attr('optionIndex');
		var vv = $('#optVal_'+baseRowId+''+inx).val();
		//alert(vv);
		if (vv == '' || vv == $(this).val()) {
			var oval = $('#optInp_'+baseRowId+''+inx).val();
			$('#optInp_'+baseRowId+''+inx).val($('#optVal_'+baseRowId+''+inx).val());
			$('.EnabledOpt_'+oval).each(function() {
				$(this).val(vv);
				$(this).text(vv);
				$(this).removeClass('EnabledOpt_'+oval);
				$(this).addClass('EnabledOpt_'+vv);
			});
		}
	});
...
$('body').on('change','.conditionalFieldsDropdown',function(event){
	...
	$.each(allOptionsFromSelectedFields, function(){
		newOptionsDropdownHTML += '<option class="EnabledOpt_'+this+'" value="' + this + '">' + this + '</option>';
	});
...
$('body').on('change','#restrictAccess', function(event){
	if ($('#restrictAccess').is(":checked")) {
...
function updateFieldNameDropdownsInLabelPrintingSettingsTable(){
	...
	//INV-284 adding dropdown values to the second drop down
	$('#objectFieldsLabelPrintingSettingsTable tbody tr .fieldNameDropdown_labelPrinting_second').each(function(){
		currentValue = $(this).val();
		$(this).html(window.fieldNamesDropdownOptionsHTML);
		$(this).val(currentValue).change();
	});
...
function changeNumberOfLabelPrinterLayoutLines(numberOfLines){
	...
	for (var i=0;i<linesToAdd;i++){
		...
		//INV-284 Adding Displaylabel text box to the labelPrintingSettings table
		$('#objectFieldsLabelPrintingSettingsTable tbody').append('<tr linenumber="'+lineNumber+'"><td>Line '+lineNumber+'</td><td><input type="text" class="fieldDisplayName_labelPrinting" jsonfield="fieldDisplayName"></td><td><div class="select-style select-style-long"><select class="fieldNameDropdown_labelPrinting"></select></td><td><select class="fieldNameDropdown_labelPrinting_second"></select></div></td></tr>')
	}
}
...
function selectConditionalFieldsFromLastSave(){
	$('#objectFieldsTable .baseRow').each(function(){
		...
		if (window.mostRecentSavedObjectData) {
			...
			$.each(window.mostRecentSavedObjectData['fields'], function () {
				if (fieldRowsForBaseRowId_fieldName == this['fieldName']) {
					...
					$('[baserowid="' + baseRowId + '"] .optionIdInput').each(function () {
						...
						if ("conditionalFieldsAndOptions" in savedObjectData_field) {
...
function addField(id){
	...
	rowHTML += '<tr class="baseRow alwaysVisible" baserowid="'+id+'" baserowidnumber="'+idNumberOnly+'">'
		...
		rowHTML += '<td class="basicSettingsCell" columnid="basicSettings">'
			rowHTML += '<div class="keyValuePair theType">'+'<label>Data Type:</label><span></span>'+'</div>'
	...
	rowHTML += '<tr class="detailsRow" baserowid="'+id+'" columnid="basicSettings">'
		rowHTML += '<td class="detailsRowTD" colspan="7">'
			rowHTML += '<div class="settingsRow">'
				rowHTML += '<div class="theType"><label for="theType_'+id+'">Data Type:</label><div class="select-style"><select name="theType_'+id+'" id="theType_'+id+'" jsonfield="theType" onChange="populateBaseRow('+idNumberOnly+');">'
					rowHTML += '<option value="text">Text</option>'
					rowHTML += '<option value="textarea">Text Area</option>'
					rowHTML += '<option value="date">Date</option>'
					rowHTML += '<option value="dropdown">Dropdown</option>'
					rowHTML += '<option value="number">Number</option>'
					rowHTML += '<option value="file">File</option>'
					rowHTML += '<option value="checkbox">Checkbox</option>'
				rowHTML += '</select></div></div>'
				rowHTML += '<input type="hidden" name="dbType_'+id+'" id="dbType_'+id+'" jsonfield="databaseType" value="text">';
				rowHTML += '<input type="hidden" name="formType_'+id+'" id="formType_'+id+'" jsonfield="formType" value="text">';
//				rowHTML += '</select></div></div>'
//				rowHTML += '<div class="dbType"><label for="dbType_'+id+'">DB Type:</label><div class="select-style"><select DISABLED name="dbType_'+id+'" id="dbType_'+id+'" jsonfield="databaseType">'
//					rowHTML += '<option value="text">Text</option>'
//					rowHTML += '<option value="date">Date</option>'
//					rowHTML += '<option value="actual_number">Number</option>'
//					rowHTML += '<option value="bool">Bool</option>'
//				rowHTML += '</select></div></div>'
//				rowHTML += '<div class="formType"><label for="formType_'+id+'">Form Type:</label><div class="select-style"><select DISABLED name="formType_'+id+'" id="formType_'+id+'" jsonfield="formType">'
//					rowHTML += '<option value="text">Text</option>'
///					rowHTML += '<option value="textarea">Text Area</option>'
//					rowHTML += '<option value="date">Date</option>'
...
//				rowHTML += '<div class="widgetName"><label for="widgetName_'+id+'">Widget Name:</label><input type="text" name="widgetName_'+id+'" jsonfield="widgetName"></div>'
			...
			rowHTML += '<div class="settingsRow multipleValuesSettingsRow">'
				...
				rowHTML += '</div>'
				rowHTML += '<div class="multipleValues_minValues" style="display:none;">'
				...
				rowHTML += '</div>'
				rowHTML += '<div class="multipleValues_maxValues" style="display:none;">'
	...
	rowHTML += '<tr class="detailsRow" baserowid="'+id+'" columnid="advancedSettings">'
		rowHTML += '<td class="detailsRowTD" colspan="7">'
			rowHTML += '<div class="advancedSettings advancedSettings_left">'
				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="inSearch" name="inSearch_'+id+'" id="inSearch_'+id+'"><label for="inSearch_'+id+'" class="checkboxLabel css-label">Field is searchable</label><br />'
				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="inImport" name="inImport_'+id+'" id="inImport_'+id+'"><label for="inImport_'+id+'" class="checkboxLabel css-label">Allow bulk importing to this field</label><br />'
				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="inTable" name="inTable_'+id+'" id="inTable_'+id+'"><label for="inTable_'+id+'" class="checkboxLabel css-label">Display field in location summary table</label><br />'
//				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="isTableLink" name="isTableLink_'+id+'" id="isTableLink_'+id+'"><label for="isTableLink_'+id+'" class="checkboxLabel css-label">Is table link</label><br />'
				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="isNameField" name="isNameField_'+id+'" id="isNameField_'+id+'"><label for="isNameField_'+id+'" class="checkboxLabel css-label">Object name field</label><br />'
				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="isBarcodeField" name="isBarcodeField_'+id+'" id="isBarcodeField_'+id+'"><label for="isBarcodeField_'+id+'" class="checkboxLabel css-label">Auto-generate barcode number</label><br />'
				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="isAmountField" name="isAmountField_'+id+'" id="isAmountField_'+id+'"><label for="isAmountField_'+id+'" class="checkboxLabel css-label">Set as \'Amount\' field</label><br />'
				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="isAmountUnitField" name="isAmountUnitField_'+id+'" id="isAmountUnitField_'+id+'"><label for="isAmountUnitField_'+id+'" class="checkboxLabel css-label">Set as \'Amount Units\' field</label><br />'
				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="isLookupField" name="isLookupField_'+id+'" id="isLookupField_'+id+'"><label for="isLookupField_'+id+'" class="checkboxLabel css-label">Registration search/link field</label><br />'
			rowHTML += '</div>'
			rowHTML += '<div class="advancedSettings advancedSettings_right">'
				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="isUnique" name="isUnique_'+id+'" id="isUnique_'+id+'"><label for="isUnique_'+id+'" class="checkboxLabel css-label">Enforce only unique values</label><br />'			
				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="required" name="required_'+id+'" id="required_'+id+'"><label for="required_'+id+'" class="checkboxLabel css-label">Required Field</label><br />'
				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="isHidden" name="isHidden_'+id+'" id="isHidden_'+id+'"><label for="isHidden_'+id+'" class="checkboxLabel css-label">Hide field from display</label><br />'
...
//				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="hideLabel" name="hideLabel_'+id+'" id="hideLabel_'+id+'"><label for="hideLabel_'+id+'" class="checkboxLabel css-label">Hidden label</label><br />'
				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="disableOnEdit" name="disableOnEdit_'+id+'" id="disableOnEdit_'+id+'"><label for="disableOnEdit_'+id+'" class="checkboxLabel css-label">Field cannot be edited</label><br />'
				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="textOnOther" name="textOnOther_'+id+'" id="textOnOther_'+id+'"><label for="textOnOther_'+id+'" class="checkboxLabel css-label">If field is a dropdown menu and has the option "Other", require users to enter a value when it is selected</label><br />'
//				rowHTML += '<input type="checkbox" class="css-checkbox" jsonfield="conditional" name="conditional_'+id+'" id="conditional_'+id+'"><label for="conditional_'+id+'" class="checkboxLabel css-label">Is conditional</label><br />'
			...
			rowHTML += '<div class="advancedSettings advancedSettings_javascriptContainer">'
				rowHTML += '<label for="templateFieldJS_'+id+'">JavaScript to run when page is saved</label>'
	...
	rowHTML += '<tr class="detailsRow" baserowid="'+id+'" columnid="formVisibilitySettings">'
		rowHTML += '<td class="detailsRowTD" colspan="7">'
			rowHTML += '<div class="settingsRow">'
				rowHTML += '<div class="checkboxContainer"><input type="checkbox" CHECKED class="css-checkbox" jsonfield="add" name="add_'+id+'" id="add_'+id+'"><label for="add_'+id+'" class="checkboxLabel css-label">Add</label></div>'
				rowHTML += '<div class="checkboxContainer"><input type="checkbox" CHECKED class="css-checkbox" jsonfield="edit" name="edit_'+id+'" id="edit_'+id+'"><label for="edit_'+id+'" class="checkboxLabel css-label">Edit</label></div>'
				rowHTML += '<div class="checkboxContainer"><input type="checkbox" CHECKED class="css-checkbox" jsonfield="view" name="view_'+id+'" id="view_'+id+'"><label for="view_'+id+'" class="checkboxLabel css-label">View</label></div>'
	...
	$('#isNameField_'+id).change(function() {
//		alert('is name: ' + $(this).prop('checked'));
		$('#isTableLink_'+id).prop('checked',$(this).prop('checked'));
		$('#isTableLink_'+id).hide();
		$('#required_'+id).prop('checked',$(this).prop('checked'));
	});
...
function makeJSON(){
	...
	//INV-284 Adding label 'layout name' and 'number of lines' values to the labelPrintingSettings
	labelPrintingSettingsObject['layoutName'] = $('#labelPrintingSettingsLayoutDropdown option:selected').text();
	labelPrintingSettingsObject['lineCount'] = $('#labelPrintingSettingsLayoutDropdown option:selected').attr('numberoflines');
	$("table#objectFieldsLabelPrintingSettingsTable tbody tr").each(function(){
		...
		//INV-284 Adding second label display values to the labelPrintingSettings
		var lineLabel2 = $(this).find('select.fieldNameDropdown_labelPrinting_second option:selected').text();
		if(lineLabel2 == "-- Leave Blank --"){
			lineLabel2 = "";
			lineLabelList = lineLabel;
		}
		else{
			lineLabelList = [lineLabel, lineLabel2]
		}
		labelPrintingSettingsObject[lineNumberLabelKey] = lineLabelList;
		
		//INV-284 Adding label display values to the labelPrintingSettings
		var lineNumberDisplayLabelKey = "line" + $(this).attr('linenumber') + "Display";
		lineDisplayLabel = $(this).find('.fieldDisplayName_labelPrinting').val();
		//INV-284 Check if the display label has a ',' init and then make it into a list
		if (lineDisplayLabel.indexOf(',') == -1){
			labelPrintingSettingsObject[lineNumberDisplayLabelKey] = lineDisplayLabel;
		}
		else{
			nameList = lineDisplayLabel.split(",");
			displayLabelList = [];
			for(var i=0; i<nameList.length; i++){
				displayLabelList.push(nameList[i])
			}
			labelPrintingSettingsObject[lineNumberDisplayLabelKey] = displayLabelList;
			console.log("DISPLAY LABEL LIST ######", displayLabelList);
		}
	});
	...
	$('#objectFieldsTable .baseRow').each(function(i,el){
		...
		field["theType"] = $('[baserowid="'+baseRowId+'"] [jsonField=theType]').val()
...
//		field["widgetName"] = $('[baserowid="'+baseRowId+'"] [jsonField=widgetName]').val()
		...
		field["isTableLink"] = $('[baserowid="'+baseRowId+'"] [jsonField=isNameField]').prop("checked"); // purposefully setting this the same as isNameField per spec
		...
		$('[baserowid="'+baseRowId+'"] table.dropdownOptionsTable > tbody > tr').each(function(){
			...
			//alert('fieldOptionId: ' + fieldOptionId);
		...
		if(field['required'] == true && field['conditional'] == true){
			...
			if (field['isNameField']) {
				field['required'] = true; // set to true if name field per requirements
			}
		}
		
//		alert("" + field['isNameField'] + " " + field['required'] + " " + field["isTableLink"]);
		
		fieldList.push(field);
	});
	theJSON["fields"] = fieldList;
	
	//alert(fieldNamesAndOptions["fieldNames"]);
...
function loadJSON(o){
	...
	for(var i=0;i<o["fields"].length;i++){
		...
		$('[baserowid=field_'+i+"] [jsonField=theType]").val(o["fields"][i]["theType"]);
		
		// set theType for older objects that don't have it..
		if (!o["fields"][i]["theType"]) {
			var tx = $('[baserowid=field_'+i+"] [jsonField=theType]");
			var dbt = $('[baserowid=field_'+i+"] [jsonField=databaseType]");
			var frt = $('[baserowid=field_'+i+"] [jsonField=formType]");

			//alert('dbt: ' + dbt.val() + ' frt: ' + frt.val());
...
function disableAndUncheck(el) {
	console.log("DISABLED & UNCHECKED MULTI");
...
function populateBaseRow(baseRowId){
	...
	// basicSettings
	var tx = $('[baserowid="field_'+baseRowId+'"] [jsonfield="theType"]');
	var tt = tx.val();
	
	frt = $('[baserowid="field_'+baseRowId+'"] [jsonfield="formType"]');
	dbt = $('[baserowid="field_'+baseRowId+'"] [jsonfield="databaseType"]');

	$('.baseRow[baserowid="field_'+baseRowId+'"] .basicSettingsCell .keyValuePair.theType span').text( $('[baserowid="field_'+baseRowId+'"] [jsonfield="theType"] option:selected').text() ).parent().addClass('makeVisible');

	if(tt && tt !== ""){
		//alert(tt);
		// set the dbType and formType

		// Show multi value checkbox for DD only. Note: we do not want this backwards compatible so we are just doing this here
		if (tt == "dropdown") {
			tx.closest(".settingsRow")
			.siblings(".multipleValuesSettingsRow")
			.css("display", "");
		}
		else{
			tx.closest(".settingsRow")
...
//		$('.baseRow[baserowid="field_'+baseRowId+'"] .basicSettingsCell .keyValuePair.theType').removeClass('makeVisible');
	}
...
function populateObjectFieldsGroupingTable(loadFromSavedSettings){
	...
	if(loadFromSavedSettings){
		if(window.mostRecentSavedObjectData){
			//INV-284 Adding the label setup status on the page for the user
			if(window.mostRecentSavedObjectData.hasOwnProperty('labelPrintingSettings')){
				if(window.mostRecentSavedObjectData['labelPrintingSettings'].hasOwnProperty('labelLayoutId') ){
					$('#objectFieldsLabelPrintingSettingsStatus').html("Label layout has been setup");
				}
			...
			$('table#objectFieldsLabelPrintingSettingsTable tbody tr').each(function(){
				...
				//INV-284 Adding labelDisplay field to the labelPrintingSettings table
				lineDisplayLabel = window.mostRecentSavedObjectData['labelPrintingSettings']['line'+domLineNumber+'Display'] // Ex: "Quantity Weighed"
				console.log("lineDisplayLabel :::::", lineDisplayLabel)
				console.log("lineDisplayLabel :::::", window.mostRecentSavedObjectData['labelPrintingSettings']['line'+domLineNumber+'Display'])
				$(this).find('.fieldDisplayName_labelPrinting').val(lineDisplayLabel);
				if (lineLabel.constructor === Array){
					//Right now we have only 2 dropdowns available
					for(var i=0; i<lineLabel.length; i++){
						if(i==0){
							...
							$(this).find('select.fieldNameDropdown_labelPrinting_second option').each(function(){ 
								if($(this).text() == lineLabel[i]){
									$(this).prop('selected',true);
									return false;
								}
					...
					$(this).find('select.fieldNameDropdown_labelPrinting option').each(function(){
