	...
	function finalizeSearch(ignoreTableFields)
	{
		L = []
		...
		for (i=0;i<groups.length ;i++ )
		{
			...
			for(j=0;j<groups[i].length;j++)
			{
				...
				if (el != null)
				{
					...
					if (el.options[el.selectedIndex].getAttribute("dbType")){
						D["dbType"] = el.options[el.selectedIndex].getAttribute("dbType");
					}else{
						D["dbType"] = false;
					}
					theFields.push(D)
	...
	function makeMongoQuery(chemIframeId,chemTableArg,chemSearchDbNameArg){
		return new Promise(function(resolve, reject) {
			...
			outerOrs = [];
			L = [];
			if($("#"+chemIframeId).length > 0){
				getChemistryEditorChemicalStructure("mycdx").then(function(chemData){
				if(chemData !== null && chemData != ""){
						...
						outerAnds.push(X);
						resolve(_mongoQuery(outerAnds));
						_mongoQuery(outerAnds);
					}
					resolve(_mongoQuery([]));
				...
				getChemistryEditorChemicalStructure("mycdx", true).then(function(chemData){
				if(chemData !== null && chemData != ""){
						el = window.parent.document.getElementById("searchTypeForSearch");
						searchType = el.options[el.selectedIndex].value;
						cdIds = restCall("/getCdIds/","POST",{'tableName':chemTableArg,'structure':chemData,'searchType':searchType});
						X = {};
						X[chemSearchDbNameArg] = {"$in":cdIds};
						outerAnds.push(X);
						resolve(_mongoQuery(outerAnds));
					}
					resolve(_mongoQuery([]));
	...
	function _mongoQuery(outerAnds, ss){
		ss = JSON.parse($("#advSearchFrame").contents().find("#savedSearchForSearch").val());
		...
		for(var i=0;i<ss.length;i++){
			...
			for(var j=0;j<group.fields.length;j++){
				c = group.fields[j];
				if(isNumber(c["value"])&&c["dbType"]=="number"){
				...
				if(c["qualifier"] == "$eq"){
					if(!isNumber(c["value"]) && c["value"] !== true && c["value"] !== false && c["dbType"]!="date"){
		...
		if(ignoreTableFields){
			outerOuter = {}
			el = parent.document.getElementById("searchObjectType");
			objectType = el.options[el.selectedIndex].value;
			outerOuter["$and"] = [{"_invType":objectType},outer];
			return outerOuter;
		}
	...
	function newCriteria(groupNumber)
	{
		...
		for (i=0;i<searchFieldList.length;i++)
		{
			...
			if(searchFieldList[i]["dbType"]){
				theOption.setAttribute("dbType",searchFieldList[i]["dbType"]);
			}
		...
		theA.onclick = (function (groupId,fieldNumber){
			return function(){
				fieldId = "value_"+groupNumber+"_"+fieldNumber;
				window.parent.blackOn();
				popup = window.parent.newPopup("barcodePopup");
				popup.style.width="300px"
				popup.style.height="400px"
				label = document.createElement("label");
				label.innerHTML = "Items/Barcodes (new line separated)";
				...
				button.style.left = "256px";
				button.onclick = (function(textarea,fieldId){
					return function(){
						theStr = textarea.value.split('\n').join(",");
						document.getElementById(fieldId).value = theStr
						el = window.parent.document.getElementById("barcodePopup");
						el.parentNode.removeChild(el);
						window.parent.blackOff();
						window.parent.document.getElementById("searchButton").onclick()
					}
				})(textarea,fieldId);
				...
				popup.appendChild(button);
				window.parent.document.getElementById("contentTable").appendChild(popup);
