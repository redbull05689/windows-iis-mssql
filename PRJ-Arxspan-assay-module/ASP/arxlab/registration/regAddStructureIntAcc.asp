...
'response.write("QQQ"&cd_id)
If cd_id <> False Then
	Set rec = server.CreateObject("ADODB.RecordSet")
	strQuery = "SELECT id, newStructure, localRegNumber, foreignRegNumber, projectName, notebookId FROM accMols WHERE included=1 and cd_id="&SQLClean(cd_id,"N","S")&" AND notebookId="&SQLClean(regNotebookId,"N","S")
	...
	Else
		...
		errorText = "This structure does not match any structures in the notebook table of contents"
	End if
...
'"  <soap:Body>"&_
'"    <GetFieldTypes xmlns=""http://sagerx.com/"">"&_
'"      <emailAddress>"&session("email")&"</emailAddress>"&_
'"    </GetFieldTypes>"&_
'"  </soap:Body>"&_
'"</soap:Envelope>"

'set xmlhttp = server.Createobject("MSXML2.ServerXMLHTTP")
'xmlhttp.Open "POST","https://cim.sagerx.com/ELNService/SageService.asmx",false
'xmlhttp.setRequestHeader "Content-Type", "text/xml"
'xmlhttp.SetTimeouts 120000,120000,120000,120000
'xmlhttp.send soapEnv
'bXML = HTMLDecode(xmlhttp.responsexml.xml)
''response.write(bXML)
'bXML = getXMLTag("GetFieldTypesResult",bXML)
'If session("debug") then
'response.write("bxml"&bXML)
'End if
'Set xmlhttp = Nothing
...
If request.Form("addStructureSubmitHidden") <> "" Then
	...
	If efields = "" Then
		...
		Select Case experimentType
			 Case 1
				experimentUrl = experimentUrl & "experiment.asp"
			Case 2
				experimentUrl = experimentUrl & "bio-experiment.asp"
			Case 3
				experimentUrl = experimentUrl & "free-experiment.asp"
			Case 4
				experimentUrl = experimentUrl & "anal-experiment.asp"
		End Select
		experimentUrl = experimentUrl & "?id=" & request.Form("experimentId")
		...
		Next
		sdFile = sdFile & "$$$$"
		'response.write("regAddStructureIntAcc: " & sdFile)
...
<%End if%>
<form action="regAddStructureIntAcc.asp?inFrame=<%=request.querystring("inFrame")%>&sourceId=<%=sourceId%>" method="post" onsubmit="document.getElementById('addStructureSubmit').disabled=true;document.getElementById('addStructureSubmit').value='WAIT';return true;">
