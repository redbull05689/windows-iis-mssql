...
<script type="text/javascript">
	function loadSearch() {
		return new Promise(function (resolve, reject) {
			finalizeSearch();
			return makeMongoQuery('<%=request.querystring("chemistryFrameId")%>', '<%=request.querystring("chemTable")%>', ['<%=request.querystring("chemSearchDbName")%>', '<%=request.querystring("chemSearchDbName2")%>']).then(function (retVal) {
				resolve(retVal);
			});
		});
	}
