...
sectionId = "prep-templates-bio-summary"
subsectionId = "prep-templates-bio-summary"
%>
...
If (session("roleNumber") <= 1 Or session("canEditTemplates")) And (templateId <> "" Or request.Form("submit") <> "" )Then
	...
	strQuery = "SELECT html, name FROM prepTemplatesBioSummary WHERE id="&SQLClean(templateId,"N","S")& " AND companyId="&SQLClean(session("companyId"),"N","S")
...
If request.Form("submit")<>"" Then
	...
	Else
		If Trim(name) <> Trim(templateName) then
			Set rec = server.CreateObject("ADODB.RecordSet")
			strQuery = "SELECT id from prepTemplatesBioSummary WHERE name="&SQLClean(Trim(name),"T","S")&" AND companyId="&SQLClean(session("companyId"),"N","S")
	...
	End If
	If errorString = "" Then
		strQuery = "UPDATE prepTemplatesBioSummary set name="&SQLClean(Trim(name),"T","S") &", html="&SQLClean(Replace(html,vbcrlf,""),"T","S")& " WHERE id="&SQLClean(request.querystring("id"),"N","S")& " AND companyId="&SQLClean(session("companyId"),"N","S")
...
<h1>Edit Biology Summary Template</h1>
<br/>
<form action="edit-prepTemplate-bio-summary.asp?id=<%=templateId%>" method="post">
<table>
<%If errorString <> "" then%>
