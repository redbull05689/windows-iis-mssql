...
If (session("roleNumber") <= 1 Or session("canEditTemplates")) And (templateId <> "" Or request.Form("submit") <> "" )Then
	...
	strQuery = "SELECT html, name FROM prepTemplatesFreeDescription WHERE id="&SQLClean(templateId,"N","S")& " AND companyId="&SQLClean(session("companyId"),"N","S")
...
If request.Form("submit")<>"" Then
	...
	Else
		If Trim(name) <> Trim(templateName) then
			Set rec = server.CreateObject("ADODB.RecordSet")
			strQuery = "SELECT id from prepTemplatesFreeDescription WHERE name="&SQLClean(Trim(name),"T","S")&" AND companyId="&SQLClean(session("companyId"),"N","S")
	...
	End If
	If errorString = "" Then
		strQuery = "UPDATE prepTemplatesFreeDescription set name="&SQLClean(Trim(name),"T","S") &", html="&SQLClean(Replace(html,vbcrlf,""),"T","S")& " WHERE id="&SQLClean(request.querystring("id"),"N","S")& " AND companyId="&SQLClean(session("companyId"),"N","S")
...
<h1>Edit Concept Description Template</h1>
<br/>
<form action="edit-prepTemplate-free-description.asp?id=<%=templateId%>" method="post">
<table>
<%If errorString <> "" then%>
...
<tr>
	<td>
		...
		<script type="text/javascript">
			...
			End If
			%>
			CKEDITOR.replace('html',{<%=ckEditorACFsetting%>,toolbar : 'arxspanToolbarPrepTemplatesAdmin',extraPlugins:'arx_onchange,arx_autoText,ajax,arx_reactantDropDowns,arx_reagentDropDowns,arx_productDropDowns,arx_solventDropDowns,arx_templateCustomDropDowns,arx_chemistryExperimentDropDowns'});
