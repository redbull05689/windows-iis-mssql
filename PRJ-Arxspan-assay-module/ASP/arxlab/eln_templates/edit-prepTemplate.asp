...
<!-- #include file="../_inclds/globals.asp"-->
	...
	<link href="<%=mainAppPath%>/css/experiment.css?<%=jsRev%>" rel="stylesheet" type="text/css" MEDIA="screen">	
...
If (session("roleNumber") <= 1 Or session("canEditTemplates")) And (templateId <> "" Or request.Form("submit") <> "" )Then
	...
	strQuery = "SELECT html, name, isGroup FROM prepTemplates WHERE id="&SQLClean(templateId,"N","S")& " AND companyId="&SQLClean(session("companyId"),"N","S")
...
If request.Form("submit")<>"" Then
	...
	End If
	If errorString = "" Then
		strQuery = "UPDATE prepTemplates set name="&SQLClean(Trim(name),"T","S") &", html="&SQLClean(Replace(html,vbcrlf,""),"T","S")& ",isGroup="&SQLClean(isGroupDBVal,"N","S")&" WHERE id="&SQLClean(request.querystring("id"),"N","S")& " AND companyId="&SQLClean(session("companyId"),"N","S")
...
<h1>Edit Chemistry Preparation Template</h1>
<br/>
<form action="edit-prepTemplate.asp?id=<%=templateId%>" method="post">
<table>
<%If errorString <> "" then%>
...
<tr>
	<td>
		...
		<script type="text/javascript">
			var editPrepTemplateHTML = "<%=replace(html,"""","\""")%>";
			matches = editPrepTemplateHTML.match(/templateGroupId=["'](\d+)["']/ig);
			if (matches){
				for (i=0;i<matches.length;i++)
				{
					matches[i] = matches[i].replace(/[^\d]/ig,"");
				}
				var cke_highest_group_id = Math.max.apply(Math,matches);
			}else{cke_highest_group_id = 0;}
			<%
