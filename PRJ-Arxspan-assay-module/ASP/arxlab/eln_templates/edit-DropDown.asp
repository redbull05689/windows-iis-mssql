...
If (session("roleNumber") <= 1 Or session("canEditTemplates")) And (ddId <> "" Or request.Form("submit") <> "" )Then
	Call getconnected
	Set rec=server.CreateObject("ADODB.RecordSet")
	strQuery = "SELECT heading, name FROM templateDropDowns WHERE id="&SQLClean(ddId,"N","S")& " AND companyId="&SQLClean(session("companyId"),"N","S")
...
If request.Form("submit")<>"" Then
	...
	Set rec = server.CreateObject("ADODB.RecordSet")
	strQuery = "SELECT id from templateDropDowns WHERE id="&SQLClean(ddId,"N","S")&" AND companyId="&SQLClean(session("companyId"),"N","S")
	rec.open strQuery,connAdm,3,3
	If rec.eof Then
	...
	If errorString = "" Then
		strQuery = "UPDATE templateDropDowns SET name="&SQLClean(name,"T","S")& ",heading="&SQLClean(heading,"T","S") & " WHERE id="&SQLClean(ddId,"N","S")
		connAdm.execute(strQuery)
		strQuery = "DELETE FROM templateDropDownOptions WHERE parentId="&SQLClean(ddId,"N","S")
		connAdm.execute(strQuery)
		For i = 1 To 50
...
<script type="text/javascript">
	function addOption()
	{
		...
		newTR.innerHTML = "<td><table><tr><td valign='middle'><input style='display:inline;width:120px;' type='text' id='option_"+nextOptionNumber+"' name='option_"+nextOptionNumber+"'></td><td valign='middle'><a href='javascript:void(0);' style='margin-left:3px;' onClick='removeOption("+nextOptionNumber+")'><img width='16' height='16' border='0' src='<%=mainAppPath%>/images/delete.gif'></td></tr></table></a></td>"
	...
	<td>
		...
		<%else%>
			<table>
				...
				Call getconnected
				Set rec = server.CreateObject("ADODB.RecordSet")
				strQuery = "SELECT value FROM templateDropDownOptions WHERE parentId="&SQLClean(ddId,"N","S")
				rec.open strQuery,conn,3,3
				If rec.eof Then%>
					<tr><td><table><tr><td><input type="text" name="option_1" id="option_1" style="width:120px;" ></td></tr></table></td></tr>				
				<%else%>
