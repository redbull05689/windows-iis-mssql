...
if request.form("configService") = "true" Then
	endpointUrl = getCompanySpecificSingleAppConfigSetting("configServiceEndpointUrl", session("companyId"))
end if

if request.form("adminService") <> "" Then
   
    if request.form("adminService") = "companyConfig" Then 
		Response.write( getCompanySpecificSingleAppConfigSetting(request.Form("url"), session("companyId")) )
		Response.end
    end if 
...
If request.Form("async") = "async" Then
	http.open request.Form("verb"),endpointUrl&request.Form("url")&"?data="&SERVER.URLEncode(request.Form("data")),True
else
