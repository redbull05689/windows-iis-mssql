...
CKEDITOR.plugins.add( 'arx_onchange',
{
...
//		editor.on( 'change', function(e) { console.log( e ) });
		...
		editor.addCommand('drawTimeStamp',{
			exec : function(editor,data){
				editor.focus();
				editor.fire( 'saveSnapshot' );

				var utcTimeFromServer = CKEDITOR.ajax.load('/arxlab/misc/getDate.asp?random='+Math.random());
				$.get( '/arxlab/misc/checkLocalizeDate.asp?random='+Math.random(), function( response ) {
				  if(response == "Use GMT: False"){
				  	var utcDate = new Date(utcTimeFromServer);
					var myDate = new Date()
					myDate.setUTCMonth(6);
				  	myDate.setUTCHours(utcDate.getHours());
				  	myDate.setUTCMinutes(utcDate.getMinutes());
				  	myDate.setUTCSeconds(utcDate.getSeconds());
				  	myDate.setUTCMonth(utcDate.getMonth());
				  	myDate.setUTCDate(utcDate.getDate());
					myDate.setUTCFullYear(utcDate.getFullYear());
					if (window.self != window.top) {
						var localizedTimestamp = window.parent.dateFormat(myDate, "m/d/yyyy h:MM:ss TT (Z)");
					} else {
						var localizedTimestamp = dateFormat(myDate, "m/d/yyyy h:MM:ss TT (Z)");
					}
				  	editor.insertHtml( localizedTimestamp );
				  }
				  else{
				  	editor.insertHtml( utcTimeFromServer );
				  }
				});
