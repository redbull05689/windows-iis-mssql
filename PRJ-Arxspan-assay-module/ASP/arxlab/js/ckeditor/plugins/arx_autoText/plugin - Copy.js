													...
													for(k=0;k<theAs.length;k++){
														if (theAs[k].className="autoFill" && theAs[k].getAttribute("templategroupid")==groupId){
															...
															if(thisFormName == 'tab_text'){
																editor.document.getById(thisId).setHtml(document.getElementById(prefixPlusJ+'_'+thisFormName).innerHTML);
															}else{
																editor.document.getById(thisId).setHtml(document.getElementById(prefixPlusJ+'_'+thisFormName).value);
															}
														}
													}
												}else{
													if(formName == 'tab_text'){
														editor.document.getById('autoFill_link_'+newId).setHtml(document.getElementById(prefixPlusJ+'_'+formName).innerHTML);
													}else{
														editor.document.getById('autoFill_link_'+newId).setHtml(document.getElementById(prefixPlusJ+'_'+formName).value);
													}
												}
											};
										...
										element.on('click', function(evt) {
											...
											for (j=0;j<30 ;j++ ){
												...
												if (document.getElementById(prefixPlusJ+"_tab")){
													...
													if(formName == 'tab_text'){
														thisLabel = document.getElementById(prefixPlusJ+"_"+formName).innerHTML
													}else{
														thisLabel = document.getElementById(prefixPlusJ+"_"+formName).value
													}
													...
													for(j=0;j<theAs.length;j++){
														...
														if(theAs[j].title == heading){
															...
															for (q=0;q<theSpans.length ;q++ ){
																if (theSpans[q].getAttribute("class") == "cke_label"){
																	theSpans[q].style.fontWeight = "bold";
																	theSpans[q].style.borderBottom = "2px solid #e3e3c7";
																}
															}
														}
													}
			...
			editor.addCommand('testcommand', {
				exec: function(editor) {
					...
					for (var i = 0; i < arr.count(); i++) {
						...
						//alert(document.getElementById('r1_tab_text').innerHTML)
						if (element.hasClass("autoFill")){
