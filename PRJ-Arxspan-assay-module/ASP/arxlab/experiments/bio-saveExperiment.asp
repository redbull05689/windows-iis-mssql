...
If Not ownsExperiment("2",experimentId,session("userId")) Then
...
End If
		...
		strQuery = "SELECT statusId FROM bioExperiments WHERE id="&SQLClean(experimentId,"N","S")
		...
		Set rs = Server.CreateObject("ADODB.RecordSet")
		strQuery = "SELECT id FROM bioExperiments_history WHERE experimentId="&SQLClean(experimentId,"N","S")
		rs.open strQuery,conn,3,3
		...
		'If Not rec.eof Then
		'	strQuery = "SET NOCOUNT ON;INSERT into bioExperiments_history(notebookId,name,details,experimentId,statusId,protocol,summary,userId,dateSubmitted,dateSubmittedServer,action,userExperimentName,revisionNumber) values(" &_
	...
	strQuery = "SELECT * FROM experimentLinks WHERE experimentType=2 and experimentId="&SQLClean(experimentId,"N","S")
	...
	Set lRec = server.CreateObject("ADODB.RecordSet")
	strQuery = "SELECT * FROM experimentRegLinks_preSave WHERE experimentType=2 and experimentId="&SQLClean(experimentId,"N","S")
	lRec.open strQuery,connAdmTrans,3,3
	Do While Not lRec.eof
	...
	strQuery = "SELECT * FROM experimentRegLinks WHERE experimentType=2 and experimentId="&SQLClean(experimentId,"N","S")
	lRec.open strQuery,connAdmTrans,3,3
	Do While Not lRec.eof
	...
	connAdmTrans.execute("DELETE FROM experimentRegLinks_preSave WHERE experimentType=2 AND experimentId="&SQLClean(experimentId,"N","S"))
	%>
	...
	strQuery = "UPDATE bioExperiments SET unsavedChanges=0 WHERE id="&SQLClean(experimentId,"N","S")
	connAdmTrans.execute(strQuery)
	...
	If newStatusId = "5" then
		a = savePDF("2",experimentId,revisionNumber,true,false,false)
