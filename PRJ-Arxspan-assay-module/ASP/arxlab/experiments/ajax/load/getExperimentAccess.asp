...
strQuery = "SELECT id FROM notebookIndexView WHERE userId="&SQLClean(userId,"N","S") & " AND typeId="&SQLClean(experimentType,"N","S")& " AND experimentId="&SQLClean(experimentId,"N","S")
crnRec.open strQuery,conn,3,3
If Not crnRec.eof Then
	'if you own the notebook the user read it
	experimentAccessString = experimentAccessString & "You are the owner of this experiment.<br/>"
End if
...
crnRec.close
strQuery = "SELECT sharerId, sharerName FROM notebookInvitesView WHERE notebookId="&SQLClean(cNotebookId,"N","S") & " AND shareeId=" & SQLClean(userId,"N","S") & " AND accepted=1 and canRead=1"
crnRec.open strQuery,conn,3,3
If Not crnRec.eof Then
	'if you have accepted a read invitation to this notebook than you 
	'the user read the notebook
	experimentAccessString = experimentAccessString & "Notebook shared by: <a href='"&mainAppPath&"/users/user-profile.asp?id="&crnRec("sharerId")&"'>"&crnRec("sharerName")&"</a><br/>"
End if

Set cgRec = server.CreateObject("ADODB.RecordSet")
strQuery = "SELECT groupId, groupName, sharerId, sharerName FROM groupPermView WHERE notebookId="&SQLClean(cNotebookId,"N","S") & " AND canRead=1 and userId=" & SQLClean(userId,"N","S")
...
If Not cgRec.eof Then
	...
	experimentAccessString = experimentAccessString & "Notebook shared with <a href='"&mainAppPath&"/table_pages/show-group.asp?id="&cgRec("groupId")&"'>"&cgRec("groupName")&"</a> by <a href='"&mainAppPath&"/users/user-profile.asp?id="&cgRec("sharerId")&"'>"&cgRec("sharerName")&"</a>.<br/>"
End if
cgRec.close
Set cgRec = nothing
...
Do While Not ttRec.eof 
	...
	Set cgRec = server.CreateObject("ADODB.RecordSet")
	strQuery = "SELECT projectId, projectName, sharerId, sharerName FROM projectInvitesView WHERE (projectId="&SQLClean(ttRec("projectId"),"N","S") & " or projectId="&SQLClean(ppId,"N","S")&") AND canRead=1 and shareeId=" & SQLClean(userId,"N","S")
	cgRec.open strQuery,conn,3,3
	If Not cgRec.eof Then
		'if the user has a usershare invute to the top level project then they can see the notebook
		experimentAccessString = experimentAccessString & "Notebook shared via project <a href='show-project.asp?id="&cgRec("projectId")&"'>"&cgRec("projectName")&"</a> by <a href='"&mainAppPath&"/users/user-profile.asp?"&cgRec("sharerId")&"'>"&cgRec("sharerName")&"</a>.<br/>"
	End if
	cgRec.close
	Set cgRec = Nothing
	
	Set cgRec = server.CreateObject("ADODB.RecordSet")
	strQuery = "SELECT groupId, groupName, projectId, projectName FROM groupProjectPermView WHERE (projectId="&SQLClean(ttRec("projectId"),"N","S") & " or projectId="&SQLClean(ppId,"N","S")&") AND canRead=1 and userId=" & SQLClean(userId,"N","S")
	cgRec.open strQuery,conn,3,3
	If Not cgRec.eof Then
		'if the user is a member of a group that has a read invite to the project hen the user can see the notebook
		experimentAccessString = experimentAccessString & "Notebook shared with group <a href='"&mainAppPath&"/table_pages/show-group.asp?id="&cgRec("groupId")&"'>"&cgRec("groupName")&"</a> via project <a href='show-project.asp?id="&cgRec("projectId")&"'>"&cgRec("projectName")&"</a>.<br/>"
	End if
	cgRec.close
	Set cgRec = nothing
	ttRec.movenext
