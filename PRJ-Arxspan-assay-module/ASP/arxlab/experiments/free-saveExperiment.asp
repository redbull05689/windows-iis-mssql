...
If Not ownsExperiment("3",experimentId,session("userId")) Then
...
End If
		...
		strQuery = "SELECT statusId FROM freeExperiments WHERE id="&SQLClean(experimentId,"N","S")
		...
		Set rs = Server.CreateObject("ADODB.RecordSet")
		strQuery = "SELECT count(*) as recordCount FROM freeExperiments_history WHERE experimentId="&SQLClean(experimentId,"N","S")
		rs.open strQuery,conn,3,3
		...
		Call addStoredProcedureArgument(args, "conceptDescription", adLongVarChar, SQLClean(experimentJSON.get("e_description"),"T-PROC","S"))
		...
		'If Not rec.eof Then
		'	strQuery = "SET NOCOUNT ON;INSERT into freeExperiments_history(notebookId,name,details,experimentId,statusId,description,userId,dateSubmitted,dateSubmittedServer,action,userExperimentName,revisionNumber) values(" &_
		...
		'				SQLClean(rec("description"),"T","S") & "," &_
	...
	strQuery = "SELECT * FROM experimentLinks WHERE experimentType=3 and experimentId="&SQLClean(experimentId,"N","S")
	...
	Set lRec = server.CreateObject("ADODB.RecordSet")
	strQuery = "SELECT * FROM experimentRegLinks_preSave WHERE experimentType=3 and experimentId="&SQLClean(experimentId,"N","S")
	lRec.open strQuery,connAdmTrans,3,3
	Do While Not lRec.eof
	...
	strQuery = "SELECT * FROM experimentRegLinks WHERE experimentType=3 and experimentId="&SQLClean(experimentId,"N","S")
	lRec.open strQuery,connAdmTrans,3,3
	Do While Not lRec.eof
	...
	connAdmTrans.execute("DELETE FROM experimentRegLinks_preSave WHERE experimentType=3 AND experimentId="&SQLClean(experimentId,"N","S"))
	...
	strQuery = "UPDATE freeExperiments SET unsavedChanges=0 WHERE id="&SQLClean(experimentId,"N","S")
	connAdmTrans.execute(strQuery)
	...
	If newStatusId = "5" then
		a = savePDF("3",experimentId,revisionNumber,true,false,false)
		a = logAction(4,experimentId,"",7)
	End if
	experimentType = "3"
