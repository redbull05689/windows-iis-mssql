...
strQuery = "SELECT DISTINCT notebookId,userId,creatorId,name,visible,lastViewed,description,fullName FROM allNotebookPermViewWithInfo WHERE userId="&SQLClean(session("userId"),"N","S")&" and visible=1 and (accepted=1 or accepted is null) "
If request.querystring("userId") <> "" Then
	strQuery = strQuery & "AND creatorId="&SQLClean(request.querystring("userId"),"N","S")& " "
End if
...
<form action="frame-show-notebooks.asp" method="get">
...
<table class="experimentsTable">
	<tr>
		...
		<th>
			<%
			sortHref = "frame-show-notebooks.asp?id="&rec("notebookId")&"&s=name&groupId="&request.querystring("groupId")
		...
		<th>
			<%
			sortHref = "frame-show-notebooks.asp?id="&rec("notebookId")&"&s=description&groupId="&request.querystring("groupId")
		...
		<th>
			<%
			sortHref = "frame-show-notebooks.asp?id="&rec("notebookId")&"&s=creator&groupId="&request.querystring("groupId")
		...
		<th>
			<%
			sortHref = "frame-show-notebooks.asp?id="&rec("notebookId")&"&s=lastViewed&groupId="&request.querystring("groupId")
	...
	%>
		<tr>
			...
			<td class="experimentCell">
				<a href="<%=mainAppPath%>/show-notebook.asp?id=<%=rec("notebookId")%>" onclick="parent.location.href=this.href"><%=rec("name")%></a>
			</td>
			<td class="statusCell">
...
<%
hrefStr = "frame-show-notebooks.asp?id="&notebookId&"&rpp="&resultsPerPage&"&s="&sortBy&"&d="&sortDir&"&q="&q&"&groupId="&request.querystring("groupId")
%>
		<tr>
