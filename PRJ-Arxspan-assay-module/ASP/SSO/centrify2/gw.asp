...
If code <> "" And queryState <> "" Then
	...
	If IsObject(sessionState) Then
		...
		If clientId <> "" And clientSecret <> "" Then
			...
			http.Open "POST", baseAuthURL&"Token/"&applicationId,False
			...
			Else
				...
				set http = server.Createobject("MSXML2.ServerXMLHTTP")
				http.Open "POST", baseAuthURL&"UserInfo/"&applicationId,False
				http.setRequestHeader "Content-Type", "application/x-www-form-urlencoded"
				...
				emailJson = JSON.stringify(uInfo)
...
If Not authError then
	...
	If Not rec.eof Then
		...
		Else
			...
			message = message & "<br>" & strResponse
			message = message & "<br>" & emailJson
			message = message & "<br>" & strQuery
			showError(message)
		End If
	Else
