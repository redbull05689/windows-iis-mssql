...
If code <> "" And queryState <> "" Then
	...
	If IsObject(sessionState) Then
		...
		If clientId <> "" And clientSecret <> "" Then
			...
			formData = formData & "&client_id="&server.urlencode(clientId)
			formData = formData & "&client_secret="&server.urlencode(clientSecret)
			formData = formData & "&scope="&server.urlencode(scope)

			set http = server.Createobject("MSXML2.ServerXMLHTTP")
			http.Open "POST", baseTokenURL & "token", False
			http.setRequestHeader "Content-Type", "application/x-www-form-urlencoded"

			http.send formData
			jsonStr = http.responseText
			Set r = JSON.parse(jsonStr)
			authToken = r.Get("id_token")
