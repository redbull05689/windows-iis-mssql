...
jsfit.fit = function (model, data, initialParams, options) {
  ...
  self.lmStep = function (params, jac) {
	...
	// Look through the minimum and maximum allowed values;
	// if a parameter is below the minimum or above the maximum
	// then set it to the minimum or maximum allowed value	
    //Patch in the fixed parameters. . . 
    for (var i=0; i<self.free.length; i++) {
	  // Move the delta value into allDelta if this
	  // parameter value is free to move (i.e. not fixed)
      if (self.free[i]) {
		  // If the parameter + the delta is less than the minimum value, set it to the minimum value
		  if(params[i] + delta[i] < self.minParamVals[i]) {
			  delta[i] = self.minParamVals[i] - params[i];
		  }

		  // If the parameter + the delta is more than the maximum value, set it to the maximum value
		  if(params[i] + delta[i] > self.maxParamVals[i]) {
			  delta[i] = self.maxParamVals[i] - params[i];
		  }
  ...
  self.calculateR2 = function() {
	var ySum = numeric.sum(self.yvals);
	var yBar = (1/self.yvals.length) * ySum;
	  
	var ssTot = 0.0;
	for(var i=0; i < self.yvals.length; i++)
		ssTot += Math.pow(self.yvals[i] - yBar, 2);
	  
	var yFun = numeric.rep([self.xvals.length], 0.0);
	for(var i=0; i < self.xvals.length; i++)
		yFun[i] = self.model(self.xvals[i], self.params);
	  
	var ssRes = 0.0;
	for(var i=0; i < self.yvals.length; i++)
		ssRes += Math.pow(self.yvals[i] - yFun[i], 2);
  ...
  self.init = function () {
    ...
    //An array indicating the minimum allowed value of the parameter 
    self.minParamVals = numeric.rep([self.npars], undefined);
    //An array indicating the maximum allowed value of the parameter 
    self.maxParamVals = numeric.rep([self.npars], undefined);
    //An array indicating if the parameter is free or fixed 
    ...
    if (self.fitterOptions.parInfo) {
      ...
      for (var i=0; i<self.npars; i++) {
        ...
        if (self.fitterOptions.parInfo[i].minVal && self.fitterOptions.parInfo[i].minVal != undefined) {
          self.minParamVals[i] = self.fitterOptions.parInfo[i].minVal;
        }
        if (self.fitterOptions.parInfo[i].maxVal && self.fitterOptions.parInfo[i].maxVal != undefined) {
          self.maxParamVals[i] = self.fitterOptions.parInfo[i].maxVal;
        }
      }
